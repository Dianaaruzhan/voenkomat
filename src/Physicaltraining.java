...
public class Physicaltraining extends JPanel{
    ...
    public Physicaltraining(){
        ...
        nextButton.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e){
                ...
                if(dialogResult == 0){
                    Packet pak = new Packet(Main.user_med,Main.user_pt,Main.user,"ADD_FINAL");
                    Main.connect(pak);
                    boolean a = chek();
                    Resultablica rt = new Resultablica();
                    rt.setId_person(Main.user.getId());
                    rt.setResult("Ne goden");
                    if(a = true)
                        rt.setResult("Goden");
                    Packet pacc = new Packet("UPDATE_UPS", rt);
                    Main.connect(pacc);
    ...
    public boolean chek(){
        int count = 11;
        boolean a = false;
        if(Main.user.getAge()>18){
            if(Main.user_med.getWeight()>100 || Main.user_med.getWeight()<40)
                count--;
            else if(Main.user_med.getHeight()<150 || Main.user_med.getHeight()>220)
                count--;
            else if(Main.user_med.getVision()>-6.0 || Main.user_med.getVision()>6.0)
                count--;
            else if(Main.user_med.getScoliosis().equals("YES"))
                count--;
            else if(Main.user_med.getFlatfeet().equals("YES"))
                count--;
            else if(Main.user_med.getWereregisteredwithadoctor().equals("YES"))
                count--;
            else if(Main.user_med.getMentalhospital().equals("YES"))
                count--;
            else if(Main.user_pt.getPullups()<10)
                count--;
            else if(Main.user_pt.getHundredmeterrun()<20)
                count--;
            else if(Main.user_pt.getThreekilometerrun()<15)
                count--;
        }
        if(count>=5)
            a = true;
